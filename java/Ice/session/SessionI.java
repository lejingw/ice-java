...
class SessionI extends _SessionDisp {
	...
	synchronized public HelloPrx createHello(Ice.Current c) {
		...
		ObjectPrx objectPrx = c.adapter.addWithUUID(new HelloI(_name, _nextId++));
		HelloPrx hello = HelloPrxHelper.uncheckedCast(objectPrx);
		_objs.add(hello);
		return hello;
	}
	...
	private java.util.List<HelloPrx> _objs = new java.util.LinkedList<HelloPrx>(); // List of per-client allocated Hello objects.
}
